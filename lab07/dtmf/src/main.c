...
void print_usage(const char *prog)
{
	printf("Usage :\n"
	       ...
	       "\t%s decode_parallel input.wav\n"
	       "\t%s decode_parallel_time_domain input.wav\n",
...
int main(int argc, char *argv[])
{
	...
	} else if (strcmp(argv[1], "decode_parallel") == 0) {
		return decode(argv[2], dtmf_decode_parallel);
	} else if (strcmp(argv[1], "decode_parallel_time_domain") == 0) {
		return decode(argv[2], dtmf_decode_parallel_time_domain);
	} else {
		print_usage(argv[0]);
		return 1;
	}
