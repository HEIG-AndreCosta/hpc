...
struct img_t *load_image(const char *path)
{
	...
	fprintf(stdout, "[%s] image %s loaded (%d components, %dx%d)\n",
		__func__, path, img->components, img->width, img->height);
#endif
...
int save_image(const char *dest_path, const struct img_t *img)
{
	...
	ret = stbi_write_png(dest_path, img->width, img->height,
			     img->components, img->data, PNG_STRIDE_IN_BYTES);
	if (ret) {
#ifdef DEBUG
		fprintf(stdout, "[%s] PNG file %s saved (%dx%d)\n", __func__,
			dest_path, img->width, img->height);
#endif
	} else {
...
struct img_t *allocate_image(int width, int height, int components)
{
	...
	img->data = (uint8_t *)calloc(
		img->width * img->height * img->components, sizeof(uint8_t));
	if (!(img->data)) {
